...
static uint16_t decoder_counter = 0;
...
uint16_t protocol_encode(const uint8_t id, const void *payload, const uint16_t payload_size, void *dest) {
    ...
    memcpy(&d[5], payload, payload_size);

    d[4] = (payload_size & 0xFF00)>>8;
    d[3] = (payload_size & 0x00FF)>>0;
    d[2] = id;

    const uint16_t crc = crc16_calculate(&d[2], payload_size + 3);

    d[1] = (crc & 0xFF00)>>8;
    d[0] = (crc & 0x00FF)>>0;

    return cobs_encode(dest, payload_size + 5);
}

bool protocol_decode(const uint8_t byte, uint8_t *id, void *payload, uint16_t *payload_size) {
    decoder_buffer[decoder_counter] = byte;
    decoder_counter++;
    decoder_counter %=sizeof(decoder_buffer);

    if(byte!=0) {
        return false;
    }

    if(decoder_counter<(5+1)) {
        decoder_counter = 0;
		return false;
	}

    const uint16_t length = cobs_decode(decoder_buffer);

    const uint16_t payload_size_p = (((uint16_t)decoder_buffer[4])<<8) | decoder_buffer[3];
    ...
    *payload_size = payload_size_p;

	const uint16_t crc_p = (((uint16_t)decoder_buffer[1])<<8) | decoder_buffer[0];
	const uint16_t crc_r = crc16_calculate(decoder_buffer + 2, *payload_size + 3);
    ...
    *id = decoder_buffer[2];
	...
	memcpy(payload, decoder_buffer + 5, *payload_size);
